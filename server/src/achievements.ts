...
const getCompletedDayKeys = (entries: AchievementEntry[]) => {
...
const getFirstStreakThresholdDate = (entries: AchievementEntry[], threshold: number) => {
...
const getRevisionThresholdDate = (entries: AchievementEntry[], threshold: number) => {
...
const getJuzThresholdDate = (
  entries: AchievementEntry[],
  onboardingMemorizedJuzList: string,
  threshold: number
) => {
  const baselineJuz = parseMemorizedJuzList(onboardingMemorizedJuzList);
  ...
  for (let index = 0; index < entries.length; index += 1) {
    const completedJuz = calculateCompletedJuz(
...
const getSurahThresholdDate = (entries: AchievementEntry[], threshold: number) => {
  for (let index = 0; index < entries.length; index += 1) {
    const completedSurahs = calculateCompletedSurahs(entries.slice(0, index + 1));
...
export const calculateAchievementStats = (
  entries: AchievementEntry[],
  onboardingMemorizedJuzList = "[]"
) => {
  const chronologicalEntries = [...entries].sort(
    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
  );
  const revisionSessions = chronologicalEntries.filter(
    (entry) => entry.manzilSaved && entry.manzil.trim()
  ).length;
